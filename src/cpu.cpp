...
static inline uint16_t next16(struct gb_state *gb_state) {
  uint8_t  b1  = gb_read_mem(gb_state, gb_state->saved.regs.pc);
  uint8_t  b2  = gb_read_mem(gb_state, gb_state->saved.regs.pc + 1);
...
static void ex_ld(struct gb_state *gb_state, struct inst inst) {
  ...
  if (IS_R16_MEM(dest) && IS_R8(src)) {
    ...
    gb_write_mem(gb_state, get_r16_mem(gb_state, dest.r16_mem), get_r8(gb_state, src.r8));
    return;
  }
  if (IS_R8(dest)) {
    uint8_t src_val;
    ...
    if (IS_R16_MEM(src)) {
      gb_spend_mcycles(gb_state, 2);
      src_val = gb_read_mem(gb_state, get_r16_mem(gb_state, src.r16_mem));
    } else if (IS_IMM16_MEM(src)) {
      gb_spend_mcycles(gb_state, 4);
      src_val = gb_read_mem(gb_state, src.imm16);
    } else if (IS_IMM8(src)) {
      gb_spend_mcycles(gb_state, 2);
      src_val = src.imm8;
    } else if (IS_R8(src)) {
  ...
  if (IS_IMM16_MEM(dest) && IS_R16(src)) {
    ...
    gb_write_mem(gb_state, dest.imm16, src_r16_val & 0x00FF);
    gb_write_mem(gb_state, dest.imm16 + 1, (src_r16_val & 0xFF00) >> 8);
    return;
  }
  if (IS_IMM16_MEM(dest) && IS_R8(src)) {
    gb_spend_mcycles(gb_state, 4);
    gb_write_mem(gb_state, dest.imm16, get_r8(gb_state, src.r8));
    return;
  }
...
static void ex_ldh(struct gb_state *gb_state, struct inst inst) {
  ...
  if (src.type == R8 && src.r8 == R8_A) {
    if (dest.type == IMM8_HMEM) {
      gb_spend_mcycles(gb_state, 3);
      gb_write_mem(gb_state, 0xFF00 + dest.imm8, r->a);
      return;
    }
    if (dest.type == R8 && dest.r8 == R8_C) {
      gb_spend_mcycles(gb_state, 2);
      gb_write_mem(gb_state, 0xFF00 + r->c, r->a);
      return;
    }
    unreachable();
  }
  if (dest.type == R8 && dest.r8 == R8_A) {
    if (src.type == IMM8_HMEM) {
      gb_spend_mcycles(gb_state, 3);
      set_r8(gb_state, R8_A, gb_read_mem(gb_state, 0xFF00 + src.imm8));
      return;
    }
    if (src.type == R8 && src.r8 == R8_C) {
      gb_spend_mcycles(gb_state, 2);
      set_r8(gb_state, R8_A, gb_read_mem(gb_state, 0xFF00 + r->c));
      return;
    }
    unreachable();
  }
...
static void ex_inc(struct gb_state *gb_state, struct inst inst) {
  GB_assert(inst.type == INC);
  GB_assert(IS_R8(inst.p1) || IS_R16(inst.p1));
...
static void ex_adc(struct gb_state *gb_state, struct inst inst) {
  GB_assert(inst.type == ADC);
  GB_assert(IS_R8(inst.p1) && inst.p1.r8 == R8_A);
  uint8_t val   = get_r8(gb_state, R8_A);
  uint8_t carry = (gb_state->saved.regs.f & FLAG_C) >> 4;
  uint8_t add;
  if (IS_R8(inst.p2)) {
    gb_spend_mcycles(gb_state, 1);
...
static void ex_add(struct gb_state *gb_state, struct inst inst) {
  GB_assert(inst.type == ADD);
  GB_assert((IS_R8(inst.p1) && inst.p1.r8 == R8_A) ||
            (IS_R16(inst.p1) && (inst.p1.r16 == R16_HL || inst.p1.r16 == R16_SP)));
  GB_assert(IS_R8(inst.p2) || IS_IMM8(inst.p2) || IS_R16(inst.p2) || IS_E8(inst.p2));
...
static void ex_sub(struct gb_state *gb_state, struct inst inst) {
  GB_assert(inst.type == SUB);
  GB_assert(IS_R8(inst.p1) && inst.p1.r8 == R8_A);
  GB_assert(IS_R8(inst.p2) || IS_IMM8(inst.p2));
...
static void ex_ccf(struct gb_state *gb_state, struct inst inst) {
  ...
  gb_spend_mcycles(gb_state, 1);
  set_flags(gb_state, FLAG_N | FLAG_H, false);
  set_flags(gb_state, FLAG_C, !(gb_state->saved.regs.f & FLAG_C));
}
...
static void ex_rl(struct gb_state *gb_state, struct inst inst) {
  ...
  uint8_t old_carry_flag = (FLAG_C & gb_state->saved.regs.f) >> 4;
...
static void ex_rst(struct gb_state *gb_state, struct inst inst) {
  ...
  push16(gb_state, gb_state->saved.regs.pc);
  gb_state->saved.regs.pc = addr;
}
...
static void ex_daa(struct gb_state *gb_state, struct inst inst) {
  ...
  uint8_t val    = get_r8(gb_state, R8_A);
  uint8_t adjust = 0;
  if ((gb_state->saved.regs.f & FLAG_N) != 0) {
    // The last inst was a subtraction.
    if ((gb_state->saved.regs.f & FLAG_H) != 0) adjust += 0x06;
    if ((gb_state->saved.regs.f & FLAG_C) != 0) adjust += 0x60;
    val -= adjust;
  } else {
    // The last inst was an addition.
    if (((gb_state->saved.regs.f & FLAG_H) != 0) || (val & 0x0F) > 0x9) adjust += 0x06;
    if (((gb_state->saved.regs.f & FLAG_C) != 0) || val > 0x99) {
      adjust += 0x60;
      set_flags(gb_state, FLAG_C, true);
    }
    val += adjust;
...
static void ex_jr(struct gb_state *gb_state, struct inst inst) {
  if (IS_IMM8(inst.p1)) {
    gb_spend_mcycles(gb_state, 3);
    gb_state->saved.regs.pc += *(int8_t *)&inst.p1.imm8;
    return;
  }
  if (IS_COND(inst.p1)) {
    if (IS_IMM8(inst.p2)) {
      ...
      if (eval_condition(gb_state, inst.p1)) {
        ...
        gb_state->saved.regs.pc += *(int8_t *)&inst.p2.imm8;
      }
      return;
    }
  }
...
void handle_interrupts(struct gb_state *gb_state) {
  ...
  uint8_t to_handle = gb_state->saved.regs.io.ie & gb_state->saved.regs.io.if_;

  // even if ime is false we still want to stop being halted once an interrupt is available
  if (to_handle) gb_state->saved.halted = false;

  if (gb_state->saved.regs.io.ime) {
    // Interupt handlers
