...
#define COMBINED_REG(regs, r1, r2) (((uint16_t)regs.r2 << 8) | ((uint16_t)regs.r1 << 0))
...
static inline void write_mem8(struct gb_state *gb_state, uint16_t addr, uint8_t val) {
  ...
  if ((addr >= IO_REG_START && addr <= IO_REG_END) || addr == 0xFFFF) {
    switch (addr) {
    case 0xFF05: gb_state->regs.io.tima = val; return;
    case 0xFF06: gb_state->regs.io.tma = val; return;
    case 0xFF07: gb_state->regs.io.tac = val; return;
